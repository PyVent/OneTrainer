...
class BaseOptimizerParamsWindowView:
    ...
    def build_dynamic_content(self, master, controller, optimizer_ui_state,
        ...
        KEY_DETAIL_MAP = {
            ...
            'riemannion_init_scale': {'title': 'Manifold Init Scale', 'tooltip': 'The fixed rank manifold does not contain zero, so the standard LoRA initialization (up weights = 0) is not a valid starting point. Zero initialized pairs are placed on the manifold with this singular value. The perturbation of the model is negligible, and the first step already rotates the subspaces into the top-r gradient directions.', 'type': 'float'},
            'riemannion_sigma_floor': {'title': 'Sigma Floor', 'tooltip': 'Lower clamp for the singular values of the adapter. Keeps the point away from the boundary of the fixed rank manifold, where the retraction becomes numerically unstable.', 'type': 'float'},
